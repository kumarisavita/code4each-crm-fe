...
const clickLimit = 1;
const maxClickLimit = 5;
const countdown = ref(0);
const limit = 5;

const buttonLabel = computed(() => {
  if (isButtonDisabled.value) {
    return `Retry in ${formatSeconds(countdown.value)} seconds`;
  } else {
...
const startCountdown = () => {
  countdown.value = 120; // Set the initial countdown time (2 seconds)
  ...
  <div class="alert-msg m-4">
    <div class="alert alert-warning alert-dismissible fade show">
      <h4 class="alert-heading">
        <i class="fa fa-exclamation-triangle" style="color: #000"></i> Email
        Verification Alert
      </h4>
      <p>
      ...
      <a
        ...
        >{{ buttonLabel }}</a
      >
      <p v-if="clickLimitReached">
        You've reached the click limit. Please connect with the support team.
      </p>
    </div>
  </div>
</template>
